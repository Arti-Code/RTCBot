...
use anyhow::Result;
use rtc::{media_stream::MediaStreamTrack, peer_connection::configuration::media_engine::MIME_TYPE_VP8, rtp, rtp_transceiver::{RTCRtpTransceiverDirection, RTCRtpTransceiverInit, rtp_sender::{RTCRtpCodec, RTCRtpCodingParameters, RTCRtpEncodingParameters, RtpCodecKind}}, shared::marshal::Unmarshal};
...
async fn async_main(name: String, ctrlc_rx: &mut Receiver<()>) -> Result<bool> {
        ...
        let video_codec = get_video_codec();
        media.register_default_codecs()?;   
        let registry = register_default_interceptors(Registry::new(), &mut media)?;
        let runtime = default_runtime().expect("no async runtime found");
        let config = RTCConfigurationBuilder::new().with_ice_servers(get_ice_servers()).build();
        ...
        let (message_tx, message_rx) = channel::<String>(1);
        let ssrc = rand::random::<u32>();
        let video_track: Arc<TrackLocalStaticRTP> = Arc::new(get_local_static_rtp(video_codec.clone(), ssrc));
...
fn get_video_codec() -> RTCRtpCodec {
    RTCRtpCodec {
...
fn get_local_static_rtp(codec: RTCRtpCodec, ssrc: u32) -> TrackLocalStaticRTP {
