...
const CAMERA_STARTER: &'static str = "127.0.0.1:8888";
...
async fn async_main(name: String, ctrlc_rx: &mut Receiver<()>) -> Result<bool> {
        ...
        let mut camera_starter = std::net::TcpStream::connect(CAMERA_STARTER)
        .expect("can't connectto  camera");
        _ = camera_starter.write_all(b"CAM START");
